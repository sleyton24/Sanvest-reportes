...
def _read_fc_eerr(path: str, norm_map: dict[str, str], asof_fid: int
                  ) -> tuple[dict[tuple[int, str], float], list[int]]:
    """Lee el bloque EERR (UF) del FC → {(fechaID, Nivel 1 exacto): valor UF} para
    los meses cerrados (202501 ≤ fechaID ≤ asof_fid). Devuelve también los meses."""
...
def apply_atempora(engine: Engine, path: str) -> dict:
    ...
    fc, months = _read_fc_eerr(path, norm_map, asof)
    if not fc:
        raise ValueError("No pude leer valores del EERR (UF) del FC; ¿hoja/estructura correcta?")

    # ELIMINAR ventas: Monto → 0 en todos los meses
    for lab in VENTA_RUBROS:
        n1 = norm_map.get(_norm(lab))
        if n1:
            for fid in months:
                if (fid, n1) in fc:
                    fc[(fid, n1)] = 0.0
    ...
    # aplicar Real (Monto) a las filas de meses cerrados
    ...
    # recalcular YTD Real: acumulado dentro del año, reiniciando en enero, por rubro.
    # Solo para los meses tocados (cerrados y contiguos desde enero).
    by_year_rubro: dict[tuple[int, str], list[int]] = defaultdict(list)
    for (fid, n1) in touched:
        by_year_rubro[(fid // 100, n1)].append(fid)
    for (yr, n1), _fids in by_year_rubro.items():
        run = 0.0
    ...
    # ELIMINAR ventas por completo: Real Y Ppto en 0 en TODOS los meses (para que la
    # operación de arriendo no arrastre la venta ni en el presupuesto).
    ...
    return {
        TABLE: {
            "filas_actualizadas": len(touched),
            "filas_insertadas": 0,
            "archivo": os.path.basename(path),
            "periodo_archivo": asof,
            "meses_actualizados": meses_txt,
            "ventas_eliminadas": list(VENTA_RUBROS),
            "nota": "ppto/YTD PPTO preservados (presupuesto congelado del Excel CIVITAS).",
        }
    }
